...
public function index() { // Display configurator
    ...
    if (!$server_id) {
        header('Location: ' . BASE_URL . '/index.php'); // Redirect to server catalog if no server selected
        exit;
    }
...
$server = $this->serverModel->getById($server_id); // Get selected server details
    ...
    if(!$server) {
        header('Location: ' . BASE_URL . '/index.php'); // Redirect to server catalog if server not found
        exit;
    }
